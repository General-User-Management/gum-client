...
class GUM extends EventEmitter {
  ...
  async addPolicy (sub, obj, act) {
    ...
    try {
      ...
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  // 1.获取用户拥有的角色
  async getImplicitRolesForUser (sub) {
    const reqObj = Object.assign({}, {
      method: 'getImplicitRolesForUser', args: [sub]
    })
    try {
      const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  // 2.获取用户拥有的角色
  async getRolesForUser (sub) {
    const reqObj = Object.assign({}, {
      method: 'getRolesForUser', args: [sub]
    })
    try {
      const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  async getUsersForRole (role) {
    const reqObj = Object.assign({}, {
      method: 'getUsersForRole', args: [role]
    })
    try {
      const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  // 查询用户分配给了那些角色
  async getFilteredNamedGroupingPolicy (sub) {
    const reqObj = Object.assign({}, {
      method: 'getFilteredNamedGroupingPolicy', args: ['g', 0, sub]
    })
    try {
      const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  // 批量移除角色权限
  async removeNamedPolicies (policies) {
    if (Array.isArray(policies)) {
      const reqObj = Object.assign({}, {
        method: 'removeNamedPolicies', args: ['p', policies]
      })
      try {
        const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
        if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
  ...
  // 移除某个用户有用的某个角色
  async deleteRoleForUser (sub, role) {
    const reqObj = Object.assign({}, {
      method: 'deleteRoleForUser', args: [sub, role]
    })
    try {
      const requestResult = await this.instance.post(this.optUrl, reqObj, this.optConfig)
      if (!!requestResult && requestResult.status === 200 && requestResult.statusText === 'OK' && requestResult.data.code === 0) return requestResult.data.data
